...
* **碎片管理與編輯：** 支援全碎片即時編輯修改、跨分類無縫轉移，確保思緒隨時可以更新迭代。
...
4. 建立 **Firestore Database**，並設定安全性規則為（請務必依使用者 UID 隔離）：
   ```javascript
   rules_version = '2';
...
### 2. 部署至 GitHub Pages
1. 將本專案 Fork 或下載至你的本地端。
2. 將你獲得的 `firebaseConfig` 替換掉 `app.js` 中的預設金鑰設定。
