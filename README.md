...
🔗 線上預覽/使用網址：https://allenphant.github.io/my-ai-brain/
...
✅ 待辦事項 (Todos)： 可打勾劃掉的生活/工作任務（支援隱藏已完成項目）。

📚 待學習/探討資源 (Learning)： 想進一步研究的知識點或教學文章。

💡 點子庫 (Ideas)： 稍縱即逝的靈感或專案構想。

📌 收藏貼文/影片 (Bookmarks)： 值得保存的外部網址、影片或社群貼文。
...
註冊一個 Web 應用程式，並取得 firebaseConfig 金鑰。

於「驗證 (Authentication)」中啟用 Google 登入。

建立 Firestore Database，並設定安全性規則為：
...
將本專案的 index.html Fork 或下載至你的本地端。

將你獲得的 firebaseConfig 替換掉 index.html 中的預設金鑰設定。

將程式碼 Push 至你的 GitHub Repository。

於 Repository 的 Settings -> Pages 中，將來源設為 main 分支並儲存。

(重要) 回到 Firebase 的 Authentication -> Settings -> Authorized domains，將你的 [GitHub帳號].github.io 加入白名單中。

3. 設定 Gemini API

使用瀏覽器開啟你部署好的 GitHub Pages 網址。

點擊右上角的「設定 (⚙️)」圖示。

前往 Google AI Studio 申請一組免費的 API Key。

將 API Key 填入網頁設定中並儲存，即可解鎖「✨ AI 魔法整理」按鈕！
