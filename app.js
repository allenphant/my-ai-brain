        ...
        const storedFbConfig = localStorage.getItem('firebaseConfig');
        if (storedFbConfig) {
            firebaseConfig = parseFirebaseConfig(storedFbConfig);
        } else if (typeof __firebase_config !== 'undefined') {
            firebaseConfig = parseFirebaseConfig(__firebase_config);
            appId = typeof __app_id !== 'undefined' ? __app_id : appId;
        }

        const isFirebaseConfigured = Boolean(firebaseConfig && firebaseConfig.apiKey && firebaseConfig.projectId);
        ...
        let updateResearchAutomationCallable = null;
        let enqueueCardResearchCallable = null;
        let resolveResearchReviewCallable = null;
        ...
        if (isFirebaseConfigured) {
            try {
                ...
                db = getFirestore(app);
                cloudFunctions = getFunctions(app, 'asia-east1');
                updateResearchAutomationCallable = httpsCallable(cloudFunctions, 'updateResearchAutomation');
                enqueueCardResearchCallable = httpsCallable(cloudFunctions, 'enqueueCardResearch');
                resolveResearchReviewCallable = httpsCallable(cloudFunctions, 'resolveResearchReview');
            } catch (err) {
        ...
        function updateFirebaseStatusBadge(isConfigured) {
            const badge = document.getElementById('firebase-status-badge');
            const clearBtn = document.getElementById('clear-firebase-btn');
            if (!badge) return;
            if (isConfigured) {
                badge.innerText = '已設定';
                badge.className = 'text-[10px] px-2 py-0.5 rounded font-bold bg-emerald-100 text-emerald-700';
                if (clearBtn) clearBtn.classList.remove('hidden');
            } else {
                badge.innerText = '未設定';
                badge.className = 'text-[10px] px-2 py-0.5 rounded font-bold bg-amber-100 text-amber-700';
