...
* **選擇性回補**：Tag 瀏覽新增「待回補」頁，只列出含單一網址且缺少 Tag 或研讀索引的卡片；可逐一勾選或全選後建立研讀佇列。
* **安全執行**：佇列逐張開啟原有預覽，必須人工確認或跳過；沿用 60 秒冷卻與 24 小時快取，冷卻時顯示倒數後自動繼續，錯誤卡片會跳過，缺少 API Key 則停止。
